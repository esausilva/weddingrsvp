...
The application also has a reCAPTCHA button to fight against bots.
...
On the back end side, the administrator can see the guest list, search for a particular name in the guest list and
delete an entry in the guest list.
...
If you get an error saying you need to re index Rsvp (+Rsvp.reindex+) run the following:

<tt>$ rake searchkick:reindex CLASS=Rsvp</tt>

<tt>$ rake db:migrate</tt>

I'm using {dotenv}[https://github.com/bkeepers/dotenv] for reCAPTCHA keys and RSVP Code. Refer to it documentation
for more info

==== Important

After cloning make sure you set up dotEnv gem before running the application and get demo
{keys}[https://github.com/ambethia/recaptcha/blob/master/demo/rails/.env] for reCAPTCHA. If not the application will
throw an error.

I disabled reCAPTCHA, if you want it just uncomment reCAPTCHA code in <b>rsvps_controller.rb</b> and <b>new.html.erb</b>
